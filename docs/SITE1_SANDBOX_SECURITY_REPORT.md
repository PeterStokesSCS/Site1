...
|---|---|---|---|---|
| R1 | **Cross-org data leakage at DB layer.** ~28 tables still use global-role RLS (`role in ('builder','office')`), no `org_id` gate. Org B's builder can read/write Org A's rows. | `supabase_rls_stage4.sql` (22 global gates), `stage4b.sql` (37). Only `tasks` is tenant-scoped. | 🔴 | Roll the Phase 3 tenant-first pattern to all tables (Phase 3 full). |
| R2 | **Public file bucket — total file leakage.** Any photo/attachment URL is world-readable, no auth, no org check. Paths are semi-guessable (`<ts>-<6 rand>`). | `src/lib/storage.js:12` `getPublicUrl`; `BUCKET="attachments"` public. | 🔴 | Private bucket + per-org path prefix + short-lived signed URLs. `supabase_migration_storage_private.sql` + `storage.js` refactor. |
| R3 | **No audit log.** No record of logins, permission-denials, CRUD, role/visibility/variation/PO events. Cannot detect or prove unauthorised access. | Only `notification_log` exists. | 🔴 | `supabase_migration_audit_log.sql` (added) + log writes at sensitive ops. |
...
| 10 | Audit-log tests | needs `audit_log` infra (migration provided) | Infra ready |
...
2. Apply `supabase_migration_storage_private.sql`; refactor `storage.js` to private bucket + org-prefixed paths + signed URLs; backfill/move existing objects.
3. Complete **Phase 3** (tenant-first RLS) across all tables; re-run `sandbox:test` until isolation is 100% green with a 2nd org.
