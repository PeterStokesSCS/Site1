...
create table if not exists variation_labour (
  id uuid primary key default gen_random_uuid(),
  project_id uuid references projects on delete cascade,
  variation_id uuid references variations on delete set null,  -- null = evidence held until matched
  work_date date,
...
-- Stable, SECURITY DEFINER helpers (bypass RLS → safe in policies; no recursion).
...
$$;
-- The new tenant tables are RLS-locked from creation (mandate: never API-exposed unguarded).
...
create policy "organisations_read" on organisations for select to authenticated using (id in (select auth_org_ids()));
create policy "organisations_admin_write" on organisations for all to authenticated using (is_org_admin(id)) with check (is_org_admin(id));
...
create policy "org_members_read_own" on org_members for select to authenticated using (user_id = auth.uid());
create policy "org_members_admin_write" on org_members for all to authenticated using (is_org_admin(org_id)) with check (is_org_admin(org_id));
...
-- Phase 2: auto-populate org_id on insert (SECURITY DEFINER; auth.uid() still resolves caller).
create or replace function set_org_from_project() returns trigger language plpgsql security definer set search_path = public as $$
begin if NEW.org_id is null and NEW.project_id is not null then NEW.org_id := (select org_id from projects where id = NEW.project_id); end if; return NEW; end $$;
create or replace function set_org_from_user() returns trigger language plpgsql security definer set search_path = public as $$
begin if NEW.org_id is null then NEW.org_id := (select org_id from org_members where user_id = auth.uid() order by (role in ('builder','office')) desc, created_at limit 1); end if; return NEW; end $$;
create or replace function set_org_from_po() returns trigger language plpgsql security definer set search_path = public as $$
begin if NEW.org_id is null and NEW.po_id is not null then NEW.org_id := (select org_id from purchase_orders where id = NEW.po_id); end if; return NEW; end $$;
create or replace function set_org_from_task() returns trigger language plpgsql security definer set search_path = public as $$
begin if NEW.org_id is null and NEW.task_id is not null then NEW.org_id := (select org_id from tasks where id = NEW.task_id); end if; return NEW; end $$;
create or replace function set_org_from_issue() returns trigger language plpgsql security definer set search_path = public as $$
begin if NEW.org_id is null and NEW.issue_id is not null then NEW.org_id := (select org_id from issues where id = NEW.issue_id); end if; return NEW; end $$;
do $$
declare t text;
begin
  foreach t in array array['project_members','tasks','hazards','issues','timesheets','daily_logs','variations','purchase_orders','commercial_items','milestones','forecast_changes','procurement_items','qa_items','defects','subbie_requests','project_photos','documents','messages','variation_labour','blockers','eot_claims','material_requests','site_visits'] loop
